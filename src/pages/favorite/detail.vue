<template>
	<view class="fav-detail-container" v-if="data.id || data.questionId">
		<!-- 题干 -->
		<view class="card">
			<view class="card-title">题目</view>
			<view class="question-content">{{ data.content }}</view>
			<image v-if="data.imageUrl" :src="picUrl(data.imageUrl)" class="question-image" mode="widthFix"
				referrer-policy="no-referrer" @click="previewImage(picUrl(data.imageUrl))" />
		</view>
		...
		<view class="card">
			...
			<view class="options-list">
				<view v-for="(opt, idx) in data.options || []" :key="idx" class="option-item">
		...
		<view class="card">
			...
			<view class="answer-highlight">{{ data.correctOption }}</view>
		</view>
		...
		<view class="card">
			...
			<view v-for="(a, ai) in data.analyses || []" :key="ai" class="platform-analysis">
		...
		<!-- 底部按钮 -->
		<view class="btn-row">
			<view class="ink-btn ink-btn-primary" @click="removeFavorite">移出收藏</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			async loadDetail() {
				this.loading = true
				try {
					const res = await getFavoriteDetail(this.questionId)
					this.data = {
						...this.data,
						...res
					}
			...
			async removeFavorite() {
				uni.showModal({
					...
					success: async (res) => {
						...
						try {
							await toggleFavorite(this.questionId)
							uni.showToast({
								title: '已移出收藏',
								icon: 'success'
							})
			...
			goPractice() {
				uni.navigateTo({
					url: `/pages/practice/practice?mode=1&examType=custom&questionId=${this.questionId}`
				})
			}
		}
	}
